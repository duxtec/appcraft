...
.. warning::
    This template is currently **inactive** (``active = False``), same as the Git Template it depends on and for the same reason — pending a migration to the framework's current internal conventions. It's hidden from ``list_templates``/``init``/``add_template`` unless ``--install-inactive`` is passed.
