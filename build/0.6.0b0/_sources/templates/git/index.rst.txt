...
.. warning::
    This template is currently **inactive** (``active = False``) pending a migration to the framework's current internal conventions. It's hidden from ``list_templates``/``init``/``add_template`` unless ``--install-inactive`` is passed, and is not recommended for new projects until it's re-activated.
